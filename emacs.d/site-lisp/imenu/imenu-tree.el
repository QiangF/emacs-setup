...
With prefix argument, select imenu tree buffer window."
  ...
  (let ((old-tree (and (local-variable-p 'imenu-tree) imenu-tree))
    ...
    (let ((win (get-buffer-window imenu-tree-buffer)))
      ...
      (unless win
        (setq win
              (display-buffer-in-side-window imenu-tree-buffer
                                             (append side-hustle-display-alist
...
(defun imenu-tree-display ()
  ...
  (let* ((widget (widget-at (1- (line-end-position))))
        ...
        (marker (widget-get widget :imenu-marker)))
    (when (markerp marker)
...
(define-key imenu-tree-mode-map (kbd "SPC") 'imenu-tree-display)

(define-key imenu-tree-mode-map (kbd "q") #'quit-window)
