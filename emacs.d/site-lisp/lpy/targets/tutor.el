...
;;   symbol.
;; - When the region is active, i shrink selection to the first symbol
;;
;; Hold =f= to move repeatedly until you reach "Skip a bit, Brother..."
(defvar weapon "hand grenade0"
  (defvar weapon "hand grenade2" (defvar weapon "hand grenade1"
                                   (message "hello"))))
...
;; deleted after all its children are gone. (C-d before ) move cursor like [)
;;
...
;;* tips
;;** region active
;; o runs the command special-lispy-other-mode
;; after mark with m, use > to extend, use d to toggle extend direction
;; y runs the command special-lispy-occur Select a line within current top level sexp.

;; M-m lispy-mark-symbol, when point is on the symbol or at the right ) enclosing it.
;; mark the third element of the list with 3m.
;; You can then mark third through fifth element (three total) with 2> or >>.
;; You can then move the selection to the last three elements of the list with 99j.
;; C-return add an empty line below sexp
;; A toggle between the beginning and current point
;; X lispy-special convolute left is a combination of C h

;; xj debug jump to definition with function argument substitution
;; Q: What does that do exactly? A: Evaluate the function arguments, jump to the function
;; definition, and store the function arguments into global variable with appropriate names.

;; xs save buffer
;; xb bind sexp to a user typed variable name
