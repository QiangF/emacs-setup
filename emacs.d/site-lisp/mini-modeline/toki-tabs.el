...
(defvar toki-tabs-project-root-function
  (lambda ()
    (when-let ((buffer-list-update-hook nil)
...
(defun toki-buffer-name (buf &optional long-name-p)
  (if (and (equal buf (current-buffer))
           (not long-name-p))
      "@"
      (let* ((buf-name (buffer-name buf))
             (buf-name-width (string-width buf-name)))
        (if (> (/ buf-name-width 2) toki-buffer-name-half-length-max)
            (concat (truncate-string-to-width buf-name toki-buffer-name-half-length-max)
                    ">"
                    (string-reverse (truncate-string-to-width
                                     (string-reverse buf-name) toki-buffer-name-half-length-max)))
            buf-name))))
  ...
  (let* ((current-buf (current-buffer))
         ...
         (get-string (lambda (buf)
                       (if (eq buf current-buf)
                           (propertize (toki-buffer-name buf)
    ...
    (when (and rest (not tab-visible-p))
      (setq rest (concat rest separator
                         (propertize (toki-buffer-name current-buf)
                                     'face 'toki-tabs-current-tab-face))))
    (if tabs
        (string-join (nconc (mapcar get-string tabs) (when rest (list rest)))
                     separator)
        (propertize (toki-buffer-name current-buf) 'face 'toki-tabs-current-tab-face))))
